...
// Self-service SSH key manager for the logged-in user's own account.
export function OwnSshKeysSection({ openAlert }) {
  ...
  async function fetchKeys() {
    try {
      const result = await Meteor.callAsync('selfService.sshKeys.list');
      setKeys(result);
    } catch (error) {
  ...
  async function handleAdd() {
    ...
    setAdding(true);
    try {
      await Meteor.callAsync('selfService.sshKeys.add', {
  ...
  async function handleRemove({ keyData }) {
    try {
      await Meteor.callAsync('selfService.sshKeys.remove', { keyData });
  ...
  return (
    <div className="w-full max-w-lg">
      <div className="rounded-xl bg-surface-card border border-border p-5">
        <h3 className="text-sm font-semibold text-fg mb-1">SSH Public Keys</h3>
        <p className="text-xs text-fg-muted mb-4">
          Add your public key to enable passwordless SSH login on domain-joined Linux hosts.
        </p>
        ...
        <div className="space-y-2 mb-4">
          ...
          <div className="flex gap-2">
            <input
              ...
              disabled={adding}
              data-e2e="profile-ssh-keys-input-label"
            ...
            <Button
              ...
              data-e2e="profile-ssh-keys-btn-add"
            >
        ...
        {loading ? (
          ...
          <div className="rounded-lg border border-border divide-y divide-border">
            {keys.length === 0 ? (
              <p className="p-4 text-sm text-fg-muted">No SSH keys added yet</p>
            ) : (
              keys.map((key) => (
                <div key={key.keyData} className="flex items-center justify-between px-4 py-2.5 gap-3">
                  ...
                  <button
                    ...
                    data-e2e="profile-ssh-keys-btn-remove"
